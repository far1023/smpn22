<?php
	...
	class Laporan_nilai extends CI_Controller
	{
		...
		function index()
		{
			...
			$kelas = $this->db->select('tbl_kelas.nama_kelas, tbl_kelas.kd_kelas, tbl_jurusan.nama_jurusan, tbl_tingkatan_kelas.nama_tingkatan')
			->from('tbl_kelas')
			->join('tbl_jurusan', 'tbl_kelas.kd_jurusan = tbl_jurusan.kd_jurusan')
			->join('tbl_tingkatan_kelas', 'tbl_kelas.kd_tingkatan = tbl_tingkatan_kelas.kd_tingkatan')
			->where('kd_kelas', $walikelas['kd_kelas'])->get();

			// $kelas = $this->db->from('tbl_kelas')->where('kd_kelas', $walikelas['kd_kelas'])->get()->num_rows();

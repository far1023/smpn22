	...
	class Siswa extends CI_Controller
	{
		...
		function edit()
		{
			if (isset($_POST['submit'])) {
				...
				$data['siswa'] = $this->db->select('siswa.*, user.id_user, user.username')->join('tbl_user user', 'user.id_user = siswa.id_user', 'left')->where('nim', $nim)->from('tbl_siswa siswa')->get()->row_array();
		...
		function nilai_siswa()
		{
			...
			if($this->session->userdata('id_level_user') == 2 && $this->session->userdata('nim') != $nim) {
				show_404();
			}

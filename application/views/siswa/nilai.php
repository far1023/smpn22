<section class="content">
    ...
    <?php
        ...
        $query = "SELECT ts.nama
                  ...
                  JOIN tbl_siswa AS ts ON tn.nim = ts.nim
                  WHERE tn.nim = ?";
        $result = $this->db->query($query, array($nim))->row_array();

        if ($result) {
            echo '<h3 class="box-title">Data Nilai Siswa ' . $result['nama'] . '</h3>';
        } else {
            echo '<h3 class="box-title">Data Nilai Siswa Tidak Ditemukan</h3>';
        }
    ?>
</div>
